...
/**
 * The total number of hash slots in a Redis Cluster. Port of platform-go's `slots.SlotCount`.
 */
public const val SLOT_COUNT: Int = 16384

/**
 * The CRC16-CCITT (XMODEM) lookup table used by Redis Cluster.
 * Reference: https://redis.io/docs/reference/cluster-spec (appendix A).
 */
private val crc16tab: IntArray =
    intArrayOf(
 ...
 * key contains `{...}` with a non-empty body, the slot is computed over that body; otherwise it's
 * computed over the whole key. Port of platform-go's `slots.SlotForKey`.
 *
 * This is the key-sharding helper the Redis backend needs to bucket keys for cluster-mode MGET/EVAL.
 * The broader `KeyGenerator`/`Topology`/`ClusterShards` machinery from `cache/redis/slots` (planned
 * even key distribution across nodes) is a `TODO(slots-keygen)` seam — not required by the backend.
 */
public fun slotForKey(key: String): Int {
    val start = key.indexOf('{')
    if (start >= 0) {
        val end = key.indexOf('}', start + 1)
        if (end > start + 1) {
            return slot(key.substring(start + 1, end))
        }
    }
    return slot(key)
}
