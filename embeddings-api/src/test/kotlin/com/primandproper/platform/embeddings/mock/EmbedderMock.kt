...
public class EmbedderMock(
    ...
    private val _generateEmbeddingCalls = mutableListOf<EmbeddingInput>()

    /** Every [EmbeddingInput] passed to [generateEmbedding], in order. */
    public val generateEmbeddingCalls: List<EmbeddingInput>
        get() = synchronized(lock) { _generateEmbeddingCalls.toList() }

    override suspend fun generateEmbedding(input: EmbeddingInput): Embedding {
        synchronized(lock) { _generateEmbeddingCalls += input }
